...
/**
 * Classe auxiliar para ler bits de um DataView,
 * simulando o comportamento do "bitio" no Go.
 */
class BitReader {
    ...
    /**
     * Lê `n` bits (máximo 32, mas aqui usaremos poucos) e retorna como número (0 a 2^n - 1).
     * Lança erro se não houver dados suficientes.
     */
 ...
 * Ex.: se mask=0b10110 e bits=0b011 => então result terá
 *      - o primeiro bit ativo de `bits` indo para o primeiro bit ativo de mask
 *      - o segundo bit ativo de `bits` indo para o segundo bit ativo de mask
 *      - ...
 */
 ...
 * @param compressedDataStart  Endereço inicial (em bytes) no buffer onde começam os dados comprimidos
...
export function unpackEnigmaFormat(
    ...
    const inline_bits = readUint8(dataView, pos); pos += 1;
    const bitfield = readUint8(dataView, pos); pos += 1;
    let inc_copy = readUint16BE(dataView, pos); pos += 2;
    const lit_copy = readUint16BE(dataView, pos); pos += 2;
