...
type Palette = [number, number, number, number][] | ((n: number) => [number, number, number, number])
...
export function getCellImageBytes(id: number, bytes: Uint8Array, pallete: Palette=normalizedPallette): Uint8ClampedArray {
